...
| **Source attribution & personas** | Each response shows source: "Dashboard data", model label, or "Web search", and is surfaced with a **persona avatar** (e.g. Neo for dashboard data, Trinity for Groq, Cortana for Tavily, Jarvis for other LLMs) to make provenance visually obvious. |
| **Country filter in Chat** | Analytics Assistant header includes the same **Country selector** as the Country Dashboard. Changing the country here updates the global `countryCode` state, so chat context and all dashboards stay in sync. |
| **Context-aware** | Uses metric metadata, selected country context, and global data. For **time-series questions** over a specific range (e.g. "from 2010 to 2024"), the client proactively fetches any missing years within the requested window before calling `/api/chat`, so rule-based summaries can cover the full overlapping years instead of a single point and combinations of metrics (GDP, population, life expectancy, inflation, debt, unemployment, labour force, poverty) without hallucinating missing series. |
...
| **Voice controls** | Optional **voice input** (microphone button in the chat box, where supported by the browser) and **per-answer voice playback** (speaker button on each assistant message) implemented via the Web Speech APIs. Both controls are designed with a professional, minimal UI that degrades gracefully when voice capabilities are unavailable. |
