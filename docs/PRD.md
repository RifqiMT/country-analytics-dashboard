...
- **Output structure** (LLM generates; UI parses and displays in this **display order**):
  1. **Porter's Five Forces Analysis** (chart) – When the LLM outputs the "Porter 5 Forces Chart Summary" block, the UI parses it and renders a **Porter's Five Forces chart** in **standard framework layout**: centre = Competitive Rivalry; top = Threat of New Entry; left = Supplier Power; right = Buyer Power; bottom = Threat of Substitution. Each force shows **exactly 5 bullet points**. Thin connectors point toward the centre. No horizontal rule (---) is used in the output.
  2. **Comprehensive Analysis** – Executive Summary (one paragraph) and the five forces narrative (exactly two paragraphs per force: Threat of new entrants, Bargaining power of suppliers, Bargaining power of buyers, Threat of substitutes, Competitive rivalry). Rendered in its own card.
  3. **New Market Analysis** – Exactly **5 summarized, concise bullet points** derived from the five forces (new market implications). Parsed from `## New Market Analysis` block; rendered in its own card.
  4. **Key Takeaways** – Exactly **5 summarized, concise bullet points** (strategic takeaways from the five forces). Parsed from `## Key Takeaways` block; rendered in its own card.
