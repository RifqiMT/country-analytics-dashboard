...
- **Porter 5 Forces**: Uses selected country context and global data (DATA_MAX_YEAR) plus a chosen ILO/ISIC industry division; supplemental web search (TAVILY first, then GROQ) enriches the analysis. The **Porter's Five Forces chart** (standard cross layout with five bullet points per force) is parsed from the LLM response and displayed above the Comprehensive Analysis when the chart summary block is present. All citations and sources are inline in the narrative.
