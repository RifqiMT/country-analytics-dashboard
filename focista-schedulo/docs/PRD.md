...
## Product benefits (why users care)

- **Plan realistically**: duration + day agenda shows what actually fits into a day.
- **Reduce noise**: recurrence shows only the next upcoming occurrence instead of flooding lists.
- **Stay organized**: projects + labels + priorities support quick triage.
- **Move fast**: voice capture + bulk actions reduce friction.
- **Build momentum**: points/levels/streak offer a lightweight progress loop.
- **Own your data**: export projects + tasks to JSON/CSV.
...
- **Positioning**: a structured planner that stays lightweight (no account required for local-first usage).
- **Trust first**: recurrence/calendar correctness is treated as a reliability feature, not “nice-to-have”.
- **Portability**: always support export; keep the data model stable and documented.
- **Progress without pressure**: gamification should motivate but not distract (simple points/levels).

## Tech guidelines

- **TypeScript-first** across backend and frontend.
- **Validation at boundaries**:
  - Backend validates inputs via Zod schemas.
  - Frontend parses voice input into structured fields but the backend remains the source of truth.
- **Identity and recurrence**:
  - Parent/child IDs are backend-enforced and standardized.
  - Migrations on startup must be deterministic and idempotent.
- **Local-first persistence**:
  - JSON persistence is acceptable for the current single-user local scope.
  - `backend/data/*.json` is intentionally not committed.
- **UX performance**:
  - Calendar segmentation and grouping should remain responsive for typical personal-scale datasets.
...
- Should completion have timestamps (required for robust weekly metrics and streak correctness)?
- Import flow: should it merge, overwrite, or create a new workspace?
- If cloud sync is added: what is the expected privacy posture and encryption model?
