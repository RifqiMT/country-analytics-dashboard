...
## Guardrail metrics (trust and correctness)

### G1 — Recurrence correctness

- **Definition**: Percentage of repeating series that display **at most one** upcoming occurrence in the active list.
- **Why it matters**: Prevents list noise and builds trust.
- **Target**: 99.5%+

### G2 — ID stability

- **Definition**: Percentage of tasks whose `parentId`/`childId` remain stable across complete/reactivate and edits.
- **Why it matters**: Ensures grouping and series logic remains coherent.
- **Target**: 99.9%+
...
## Instrumentation plan (future)

To support the metrics above reliably, consider adding:

- `createdAt` timestamp on tasks (backend-assigned)
- `completedAt` timestamp on completion transitions
- event logs for:
  - voice capture used
  - calendar view opened and day agenda opened
  - export started/succeeded
