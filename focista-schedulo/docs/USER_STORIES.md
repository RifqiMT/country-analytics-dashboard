...
- “Task” means a persisted backend task record.
- “Series” means a repeating task identity group (shared `parentId`).
- “Occurrence” means a specific instance in a series (has `childId`).
...
- After save, the task appears in the list and (if scheduled) in the calendar.
 - Task is created via API validation (invalid payloads are rejected).
...
- Duration changes propagate to series occurrences where applicable.
 - Project association updates are reflected across list and calendar views.
...
- User can still manually correct fields before saving.
 - Voice failure is non-blocking (user can type and save normally).
...
- Upcoming occurrence behaves like a normal task: can be opened, edited, and completed.
 - Cancelling/deleting a recurring item does not cause it to reappear.
...
- Multi-day tasks appear on every day they span.
 - Timed tasks appear at their start time with height proportional to duration.
