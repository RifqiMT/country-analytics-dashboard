...
- **Friendly name**: Has scheduled start time
- **Definition**: Whether a task has an explicit `dueTime`.
- **Formula**: `Boolean(task.dueTime)`
- **Location**: Calendar month view and day agenda rendering
...
- **Friendly name**: Default duration (minutes)
- **Definition**: Fallback duration used when `durationMinutes` is not set.
- **Formula**:
  - if `hasTime` then 30
  - else 1440 (all-day)
- **Location**: Calendar segmentation and day agenda block sizing
- **Source of truth**: Frontend-derived
- **Example**: `30`

### `startMin` / `endMin`

- **Friendly name**: Segment start/end minutes
- **Definition**: Start/end for a calendar segment in “minutes since midnight” on the segment day.
- **Formula**:
  - `startMin = hours(start) * 60 + minutes(start)`
  - `endMin = hours(end) * 60 + minutes(end)` (clamped; can be 1440)
- **Location**: `CalendarEntry` rendering in month view and day agenda
- **Example**: `startMin=570` (09:30), `endMin=660` (11:00)

### `toISODateLocal(date)`

- **Friendly name**: Local ISO date string
- **Definition**: Converts a local Date to a stable `YYYY-MM-DD` string for grouping and comparisons.
- **Location**: Calendar grouping, recurrence comparisons
- **Source of truth**: Frontend utility
- **Example**: `2026-03-18`
...
- **Definition**: Count of tasks counted as completed “today” by backend stats logic.
- **Formula**: backend-derived; for robust analytics this should use completion timestamps (planned enhancement).
