  ...
  const execTableRows = useMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        sortInterp: res.slope !== null ? "1-unit increase in X predicts change in Y" : "—",
        metric: "Beta (slope)",
        value: res.slope !== null ? res.slope.toExponential(2) : "—",
        interp: res.slope !== null ? "1-unit increase in X predicts change in Y" : "—",
      },
    ];
  }, [res]);
    ...
    <div className="space-y-8">
              ...
              <label className="flex cursor-pointer items-start gap-3">
                ...
                <div>
                  ...
                  <p className="mt-0.5 text-xs text-slate-500">
                    Remove points &gt;1.5×IQR from Q1/Q3 (univariate on X and Y)
                  </p>
                </div>
              </label>
            </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
        ...
        <div className="mt-6 grid gap-6 sm:grid-cols-2">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">X axis</p>
            <select
              value={xId}
          ...
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Y axis</p>
            <select
              value={yId}
      ...
      {res && !loading && (
        <>
          <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
            ...
            <p className="mt-1 text-sm text-slate-500">
              Statistical summary and interpretation for the selected pair: {labelX} (X) vs {labelY}{" "}
              (Y). Years: {startYear}–{endYear} ({yearCount} years, each country–year is a point).
            </p>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
              ...
              <p className="mt-2 text-sm leading-relaxed text-slate-600">
                There is a {res.correlation !== null ? strengthLabel(res.correlation) : ""} linear
                relationship across {formatCompactCount(res.n)} points: higher X tends to higher Y.{" "}
                {res.nIqrFlagged > 0 &&
                  `${res.nIqrFlagged} point(s) flagged as IQR outliers (1.5×IQR rule).`}
              </p>
              {res.slope !== null && (
                <p className="mt-2 text-sm font-medium text-slate-700">
                  A 1-unit increase in X predicts {res.slope.toExponential(2)} change in Y (p ={" "}
                  {res.pValue ?? "—"}).
                </p>
              )}
            </div>
          ...
          <div className="space-y-4">
            <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
              ...
              <p className="mt-3 text-sm leading-relaxed text-slate-600">
                ...
                health systems, geography) may confound the relationship. Example: higher income per
                capita is associated with better health outcomes, but both may be driven by
                third factors.
              </p>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
              ...
              <p className="mt-3 text-sm leading-relaxed text-slate-600">
                {res.slope !== null && (
                  <>
                    A 1-unit increase in X ({xId}) predicts a change of {res.slope.toExponential(2)}{" "}
                    in Y ({yId}) (p={res.pValue ?? "—"}). {strengthLabel(res.correlation ?? 0)}{" "}
                    correlation. Use for hypothesis generation; confirm with subgroup analysis and
                    robustness checks.
                  </>
                )}
              </p>
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5">
              ...
              <p className="mt-2 text-sm text-slate-600">
                Working hypothesis for <strong>{labelX}</strong> (X) vs <strong>{labelY}</strong> (Y)
                across {yearCount} years ({startYear}–{endYear}):
              </p>
              <ul className="mt-4 list-disc space-y-2 pl-5 text-sm leading-relaxed text-slate-600">
                <li>
                  <strong>Statistical trend:</strong> Higher X values tend to have higher Y values
                  based on scatter and Pearson statistics ({formatCompactCount(res.n)} points with valid
                  data).
                </li>
                <li>
                  <strong>Strategic implication:</strong> Shifts in X may be informative of movements
                  in Y within target segments.
                </li>
                <li>
