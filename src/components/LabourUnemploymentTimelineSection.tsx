...
const LABOUR_LEGEND_LABELS: Record<string, string> = {
...
export function LabourUnemploymentTimelineSection({
  ...
  const [selectedMetricIds, setSelectedMetricIds] = useState<MetricId[]>(LABOUR_METRIC_IDS);
  ...
  const handleDownloadChartPng = async () => {
    ...
    try {
      ...
      console.error('Labour chart export failed:', err);
    }
  };

  const handleDownloadTableCsv = () => {
    const rows: string[] = [];
    const header = ['Period', ...LABOUR_METRIC_IDS.map((id) => labelByMetricId[id] ?? id)];
        ...
        <div className="metric-toggle-row">
          {LABOUR_METRIC_IDS.map((id) => (
            ...
            >
              ...
              {LABOUR_LEGEND_LABELS[id] ?? labelByMetricId[id] ?? id}
            </button>
          ))}
        </div>
