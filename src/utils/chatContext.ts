...
`;
  ...
  const cutoffRule = `Today is ${today}. For general-knowledge questions (e.g. location/geography, leaders, presidents, independence day, current events), frame your qualitative answer **as of today** using the most current information you know (including web search when available). Consider recent elections, leadership changes, or events that may have occurred since your training. For any numeric macro metrics, you MUST only use values from the dataset context (which is limited to ${DATA_MIN_YEAR}–${DATA_MAX_YEAR}) and clearly explain when the latest available numeric data is older than today. Do NOT say "my knowledge cutoff". Do NOT mention "Dashboard data". Instead, add a Wikipedia link for verification, e.g. "For more: [Indonesia – Wikipedia](https://en.wikipedia.org/wiki/Indonesia)". Keep responses confident and concise.`;
  const intro = compact
...
- **Schools & universities metrics**: For primarySchoolCount, secondarySchoolCount, and tertiaryInstitutionCount, ONLY use the values provided in the context (they are estimated counts, derived from enrollment using fixed denominators 250/500/5,000). If any of these values are missing (null/undefined/NaN) for a country/year, you MUST display them as "N/A – no data available for this country/year" and explicitly say that the platform has no data; do NOT guess, infer, or hallucinate counts of schools or universities.
- **Year-specific numeric data**: Data is available for ${DATA_MIN_YEAR}–${DATA_MAX_YEAR}. When the user asks for a year (e.g. "GDP in 2020", "from 2023"), use that year's data from the context. Never substitute a different year and never invent numeric values beyond ${DATA_MAX_YEAR}. If the question mentions "now", "current", or "currently" and the requested numeric year is after ${DATA_MAX_YEAR}, explain that the latest numeric data is for ${DATA_MAX_YEAR} and answer any additional parts qualitatively "as of today".
...
- **Data recency**: Today is ${today}. Provide qualitative explanations **as of today** for general-knowledge questions (leaders, politics, business environment, geography, current events), using the most recent information you know and any web-search context provided. For any numeric macro metrics (GDP, population, inflation, debt, etc.), you MUST stay within the dataset window ${DATA_MIN_YEAR}–${DATA_MAX_YEAR} and clearly signal when the latest numeric value is older than today. Do NOT say "my knowledge cutoff". Do NOT mention "Dashboard data". Add a Wikipedia link for verification, e.g. "For more: [Country name – Wikipedia](https://en.wikipedia.org/wiki/Country_name)".`;
...
- **NO HALLUCINATED SCHOOL / UNIVERSITY COUNTS**: When users ask about the **number of schools or universities** (or "institutions") for any country or in any comparison or ranking, you must ONLY use the primarySchoolCount, secondarySchoolCount, and tertiaryInstitutionCount values from the context. If they are missing for a given country/year, answer with N/A and an explicit note that the dashboard has no data for that metric. Do NOT estimate from GDP, population, or any external knowledge. Do NOT interpolate from other countries. Do NOT fabricate numbers.
